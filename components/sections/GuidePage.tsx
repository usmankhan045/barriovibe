      ...
      return (
        <div className="mt-12 first:mt-0">
          ...
          <div className={section.heading ? 'mt-5' : ''}>
            {section.body.map((p) => (
              <p key={p} className="mt-4 text-[16px] leading-[1.7] text-ink-body first:mt-0">
                {p}
              </p>
            ))}
          </div>
    ...
    case 'list':
      return (
        <div className="mt-12">
          <h2 className="font-display text-h3 text-ink">{section.heading}</h2>
          {section.intro && (
            <p className="mt-5 text-[16px] leading-[1.7] text-ink-body">{section.intro}</p>
          )}
            ...
            {section.items.map((item) => (
              <li key={item} className="flex items-start gap-3.5">
                <span
                  ...
                  aria-hidden="true"
                />
                <span className="text-[15.5px] leading-[1.65] text-ink-strong">{item}</span>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ol className="mt-6 flex flex-col gap-5">
            {section.steps.map((step, i) => (
              <li key={step.title} className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-[15.5px] font-bold text-ink-strong">{step.title}</h3>
                  <p className="mt-1.5 text-[15px] leading-[1.65] text-ink-body">{step.body}</p>
                </div>
              </li>
            ))}
          </ol>
    ...
    case 'note':
      return (
        ...
        >
          ...
          <p className="mt-3 text-[15px] leading-[1.65] text-ink-strong">{section.body}</p>
        </aside>
      );
    ...
    case 'calculator': {
      ...
      return (
        <div className="u-tile mt-10 p-6 md:p-7">
          <h2 className="font-display text-h3 text-ink">{section.heading}</h2>
          <p className="mt-3 text-[15px] leading-[1.65] text-ink-body">{section.body}</p>
          <Link href={toolHref(tool)} className="u-arrow-link mt-5 inline-flex text-caption">
            {tool.navLabel}
    ...
    <main id="main" tabIndex={-1}>
      ...
      <section className="pb-4 pt-10 md:pt-14">
        <Container>
          ...
          <div className="mt-8 max-w-[68ch]">
            ...
            <h1 className="mt-3 font-display text-h1 text-ink">{guide.title}</h1>
            <Rule className="mt-6" />
            <p className="mt-6 text-body-lg text-ink-body">{guide.answer}</p>
          </div>
      ...
      {siblings.length > 0 && (
        <Section tight>
          <Container>
            <Reveal>
              <SectionHeading eyebrow="Also here" lines={['Related']} accent="guides" />
            </Reveal>
            ...
            <div className="u-tile flex flex-col gap-8 p-8 lg:flex-row lg:items-center lg:justify-between lg:p-10">
              <div className="max-w-[52ch]">
                ...
                <p className="mt-3 text-[15px] leading-[1.65] text-ink-body">
                  We file returns through IRIS every day. If the answer above raises a
                  question about your own case, that is the conversation to have before
